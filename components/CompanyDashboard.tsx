...
const CompanyDashboard: React.FC<CompanyDashboardProps> = ({ user, onLogout }) => {
  ...
  const handleSubmitVessel = async (e: React.FormEvent) => {
    ...
    const username = vesselFormData.username.toUpperCase().replace(/\s+/g, '_');

    if (editingShipId) {
      await db.updateShip(user.companyId, editingShipId, {
        shipName: vesselFormData.name,
        username,
        password: vesselFormData.password
      });
    } else {
      await db.createShip(user.companyId, vesselFormData.name, username, vesselFormData.password);
    }
  ...
  const handleUnlock = async (shipId: string) => {
    if (confirm('Unlock this report? This will allow the vessel to edit it again (Revert to Draft).')) {
      await db.updateReportStatus(user.companyId, monthKey, shipId, ReportStatus.DRAFT);
      loadData();
    }
  };
  ...
  const updateReviewItem = (catId: string, idx: number, field: keyof ProvisionItem, value: any) => {
    ...
    const totalPersonDays = (reviewReport.crewCount * daysInMonth) + ((reviewReport.guestMeals || 0) / 3);
  ...
  const darkInputClasses =
    "w-full bg-slate-900 text-white border border-slate-700 rounded-2xl px-5 py-4 text-sm font-bold outline-none focus:ring-4 focus:ring-blue-500/20 placeholder-slate-500 transition-all";
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <button
            onClick={() => {
              ...
              setVesselFormData({ name: '', username: '', password: '' });
              setIsModalOpen(true);
            }}
            ...
            <form onSubmit={handleSubmitVessel} className="space-y-6">
              ...
              <div>
                ...
                <input
                  required
                  className={darkInputClasses}
                  placeholder="ADVANTAGE_SUMMER"
                  value={vesselFormData.username}
                  onChange={(e) => setVesselFormData({ ...vesselFormData, username: e.target.value })}
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className={darkInputClasses}
                  placeholder="••••••••"
                  value={vesselFormData.password}
                  onChange={(e) => setVesselFormData({ ...vesselFormData, password: e.target.value })}
                />
              </div>
                      ...
                      <td className="px-8 py-10">
                        <div>
                          ...
                          <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">
                            USER: {ship.username} • PW: <span className="text-blue-500">{ship.password}</span>
                          </p>
                        </div>
                      </td>
                      ...
                      <td className="px-8 py-10">
                        <div className="flex gap-4">
                          {!ship.isArchived && (
                            <button
                              onClick={() => {
                                setEditingShipId(ship.shipId);
                                setVesselFormData({ name: ship.shipName, username: ship.username, password: ship.password });
                                setIsModalOpen(true);
                              }}
                              className="bg-slate-100 text-slate-500 p-4 rounded-3xl hover:bg-slate-200 transition-all shadow-sm"
                            >
                          ...
                          {isSubmitted && !ship.isArchived && (
                            <button
                              onClick={() => handleUnlock(ship.shipId)}
                              className="bg-amber-100 text-amber-700 p-4 rounded-3xl hover:bg-amber-200 transition-all shadow-xl shadow-amber-50"
                              title="Unlock Report"
                            >
                          ...
                          <button
                            onClick={() => db.updateShipArchived(user.companyId, ship.shipId, !ship.isArchived).then(loadData)}
                            className={`p-4 rounded-3xl transition-all shadow-sm ${
                              ship.isArchived ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200' : 'bg-rose-50 text-rose-300 hover:bg-rose-100 hover:text-rose-600'
                            }`}
                          >
